<script setup>
  ...
  const trigger = () => {
    ...
    gsap.to(cardRef.value, { top:'calc(50vh)', left:'calc(25vw)', duration: .5, ease: 'power1.inOut' })
  ...
  watch(() => props.activeCardId, (newVal, oldVal) => {
    ...
    if (newVal === null && oldVal === props.cardId) {
      gsap.to(cardRef.value, { duration: .5, left: savedPosition.value.left, top: savedPosition.value.top, ease: 'power1.inOut' })
